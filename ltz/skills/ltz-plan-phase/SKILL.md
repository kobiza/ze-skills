...
You are executing the "/ltz:ltz-plan-phase" protocol. Your job is to act as the Systems Analyst. Do NOT write application code.
...
3. **Identify Target:** Look at the `## Progress` table and find the first phase with status `backlog`.
   - If no `backlog` phase exists, tell me: *"All phases are already planned. Run `/ltz:ltz-execute` to start the next ready phase."* and stop.
...
5. **Review Loop:** Print an inline summary of what was drafted: the phase name, acceptance criteria, number of atomic tasks, and the verification command(s). Then use `AskUserQuestion` with the question *"Does this phase plan look right?"* and options:
   ...
   - **If `approve`:** In `plan.md`, find the `## Progress` table and update the current phase's status from `backlog` to `ready-for-dev`. Then confirm *"Tasks approved. Run `/ltz:ltz-execute` to implement this phase."* and stop.
