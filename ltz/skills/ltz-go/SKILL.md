...
You are executing the "/ltz:ltz-go" protocol. Your job is to drive an existing plan to completion by delegating to existing skills. Do NOT create new plans or duplicate planning/execution logic.

## Step 1 — Resolve Active Plan

Read `.plan/.active` to get the current plan name.

- If the file is missing or empty: *"No active plan. Run `/ltz:ltz-plan` to start one or `/ltz:ltz-list` to browse."* Stop.
- If a plan name is found, use `AskUserQuestion` with *"Active plan: **[name]**. Continue with this plan?"* and options:
  ...
  - `Cancel`
- **If "No":** Invoke `/ltz:ltz-choose-plan`, then restart from Step 1.
- **If "Cancel":** Stop.
...
**If task plan:**
Invoke `/ltz:ltz-execute` — it handles the full task implementation, verification, and status update. Then go to Step 4.

**If feature plan:**
Loop over each phase that is NOT `done`, in order:

1. **If the phase is `backlog`:** Invoke `/ltz:ltz-plan-phase` — it finds the next backlog phase, details it with atomic tasks and verification commands, and runs a review loop.
...
2. **If the phase is `ready-for-dev`:** Invoke `/ltz:ltz-execute` — it runs all ready-for-dev phases, marks them done.
