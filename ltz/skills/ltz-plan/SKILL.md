...
You are executing the "/ltz:ltz-plan" protocol. Your job is to classify the request and delegate to the right planning skill. Do NOT write application code or duplicate planning logic — let the target skill handle everything.
...
1. **Classify:** Evaluate the request (from ARGUMENTS) and classify it as **task** or **feature**:
   ...
   - Announce the classification briefly: *"Classified as **task** — delegating to `/ltz:ltz-plan-task`."* or *"Classified as **feature** — delegating to `/ltz:ltz-plan-feature`."*

2. **Delegate:** Invoke the matching skill using the `Skill` tool, passing the original ARGUMENTS:
   - **If task:** Invoke `/ltz:ltz-plan-task` with the original ARGUMENTS.
   - **If feature:** Invoke `/ltz:ltz-plan-feature` with the original ARGUMENTS.
