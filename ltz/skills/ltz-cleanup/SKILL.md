...
You are executing the "/ltz:ltz-cleanup" protocol.
...
2. **Resolve Active Plan:** Read `.plan/.active` to know which plan is currently focused.
   - If a plan name is found, use `AskUserQuestion` with the question *"Active plan: **[name]**. Delete this plan or choose a different one?"* and options:
     ...
     - `Cancel`
   - **If "Choose a different plan":** Invoke `/ltz:ltz-choose-plan`, then resume from step 2.
...
7. **Confirm:** Tell me: *"Plan **[plan-name]** has been deleted."* If active was cleared, add: *"Run `/ltz:ltz-list` to set a new active plan."*
