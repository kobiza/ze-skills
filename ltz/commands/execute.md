...
**Write ownership:** This skill writes ONLY to `.plan/<feature>/progress.md` (updates) and `.plan/<feature>/findings.md` (appends). It MUST NOT modify `.plan/<feature>/plan.md` or any file under `.plan/<feature>/phases/`. Those are owned by the planning skills.
...
2. **Collect Ready Phases:** Read `.plan/[active-name]/progress.md` and collect the full ordered list of `ready-for-dev` rows. Capture each row's 1-based index as the phase number `N`. For each ready row, also read `.plan/[active-name]/phases/phase-N.md` to obtain the atomic tasks and verification commands.
...
3. **Loop — Execute Each Phase:** For each phase in the collected list (in order):
   a. **Execute:** Implement the atomic tasks defined in `phases/phase-N.md` step-by-step. Follow the global "Ask User Question" rule to get approval before modifying files.
   b. **3-Strike Error Protocol:** If you encounter the exact same error 3 times:
      - **STOP immediately.** Append the error, what you tried, and your hypothesis to `.plan/[active-name]/findings.md`.
   ...
   c. **Verify & Update:** Run the verification commands for this phase. If they pass:
      - In `progress.md`, update the row for phase N — set status to `done` and the Done column to `[x]`. Do NOT modify `plan.md`.
